...
function downloadPDF(evaluationData) {
    ...
    doc.setFontSize(85);
    doc.text("John Doe", pageWidth / 2, 100, { align: 'center' });

    // Add quiz completion details with default fonts
    doc.setFont("Helvetica");
    doc.setFontSize(15);
    doc.text("Has successfully passed the theoretical driving course on January 1, 2024", pageWidth / 2, 120, { align: 'center' });
    doc.text("with a quiz result of 90%, earning Quiz Passing ID 1010101", pageWidth / 2, 130, { align: 'center' });

    // Signature fields
    doc.setFont("Helvetica");
    doc.setFontSize(24);
    doc.text("Samira Hadid", 30, 160);
    doc.line(30, 162, 81, 162);  // Signature line

    doc.setFont("Helvetica");
    doc.setFontSize(17);
    doc.text("TDC Instructor", 34, 170);

    // Add logo and adjust size
    const logoBase64 = 'Assets/logo.png';  // Base64 of the logo
    doc.addImage(logoBase64, 'PNG', 130, 150, 30, 30);

    // Second signature fields
    doc.setFont("Helvetica");
    doc.setFontSize(24);
    doc.text("Aaron Loeb", 215, 160);
    doc.line(215, 162, 259, 162);  // Signature line

    doc.setFont("Helvetica");
    doc.setFontSize(17);
    doc.text("Admin", 228, 170);
